...
#ifdef ACCEL
volatile int* vec_size = (volatile int*)0xc4003000;
volatile float* vec_weight = (volatile float*)0xc4001000;
volatile float* result = (volatile float*)0xc4002000;
#endif
...
int neuronet_eval(NeuroNet *nn, float *images)
{
    ...
    {
        #ifdef ACCEL
            *vec_size = nn->n_neurons[layer_idx-1]; // Feed the size of a vector (without bias)
            p_neuron = nn->previous_neurons[neuron_idx];
            volatile float* vec_neuron = (volatile float*)0xc4000000;
            for(idx = 0; idx < nn->n_neurons[layer_idx-1]; idx++) *vec_neuron++ = *p_neuron++; // Feed vector A
        #endif
        for (idx = 0; idx < nn->n_neurons[layer_idx]; idx++, neuron_idx++)
        {
            ...
            {   
                #ifdef ACCEL
                    *vec_weight = *p_weight++; // Feed an element of vector B (weight)
                #else
            ...
            #ifdef ACCEL
                *vec_weight = *p_weight; // Feed bias
                inner_product = *result; // Read result
            #else
